...
Frustum::ObjectLocation Frustum::Test_AABB(const AABB &aabb) const
{
	...
	for(int p = 0; p < 6; p++)
	{
		...
		if(m_planes[p].SignedDistanceTo(aabb.GetVertexP(Vec3f(m_planes[p].a, m_planes[p].b, m_planes[p].c))) < 0.0f)
			return outside;

		// If positive vertex is inside
		else if(m_planes[p].SignedDistanceTo(aabb.GetVertexN(Vec3f(m_planes[p].a, m_planes[p].b, m_planes[p].c))) < 0.0f)
			location = intersect;
	}
...
Vec3f Frustum::GetCorner(size_t index) const
{
	assert(index < 8);
