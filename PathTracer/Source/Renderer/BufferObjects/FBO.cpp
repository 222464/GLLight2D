...
FBO::FBO(unsigned int width, unsigned int height, bool depthAttachment, GLuint internalFormat, GLuint textureFormat, GLuint dataType)
	: m_created(false),
...
void FBO::Create_FromSetParams(bool depth, GLuint internalFormat, GLuint textureFormat, GLuint dataType)
{
#ifdef DEBUG
	int result;
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_texID, 0); // The 0 is for mip map levels, we aren't using any
...
void FBO::Create(unsigned int width, unsigned int height, bool depthAttachment, GLuint internalFormat, GLuint textureFormat, GLuint dataType)
{
	m_width = width;
	m_height = height;
